...
const { sanitizeLabelName } = require("./name-sanitize");
...
function minLabelBoxHeight(label) {
    let minH = label.lineHeight || label.fontSize || 24;
    const outline = label.outline;
    if (outline && outline.width > 0) minH += outline.width * 2;
    const shadow = label.shadow;
    if (shadow) minH += Math.max(0, shadow.offsetY || 0) + (shadow.blur || 0);
    return minH;
}

function ensureLabelHeight(ir, label) {
    const text = String(label.text || "");
    if (text.indexOf("\n") >= 0) return;
    const minH = minLabelBoxHeight(label);
    const oldH = ir.height || 0;
    if (minH <= oldH) return;
    // verticalAlign TOP：扩高时固定顶边，避免标题整体下移。
    ir.y -= (minH - oldH) / 2;
    ir.height = minH;
}
...
function buildLabelLayout(node) {
    ...
    // 单行标题按 PSD 字形框顶对齐；段落/换行仍顶对齐。
    const verticalAlign = tight || hasNewline || wrap ? 0 : 1;
    // NONE(0) 会按字形缩小节点；锚点在中心时，左对齐标题（如 Game Objective:）会整体右移。
    // CLAMP(1) 保持 PSD 文本框；仅居中单行标题才允许 NONE。
    const overflow = tight && node.align === "center" ? 0 : 1;
    let yShift = 0;
    if (verticalAlign === 0) {
...
function walkDescNode(node, parentAbs, warnings, usedLabelNames) {
    const pos = relPos(node, parentAbs);
    const kind = toKind(node.type);
    const labelNameSource = kind === "label"
        ? (node.text || node.name || "label")
        : "";
    const nodeName = kind === "label"
        ? sanitizeLabelName(labelNameSource, usedLabelNames)
        : (node.name || "node");
    const ir = {
        name: nodeName,
    ...
    for (const child of orderedChildren(node)) {
        ir.children.push(walkDescNode(child, abs, warnings, usedLabelNames));
    }
    return ir;
}
...
function buildIr(rootDesc) {
    ...
    for (const child of orderedChildren(rootDesc)) {
        root.children.push(walkDescNode(child, parentAbs, warnings, usedLabelNames));
    }
