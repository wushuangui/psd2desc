...
const { copySystemFontTo } = require("./system-fonts");
...
function buildImageMeta3x(uuid, baseName, width, height, imported) {
    const hw = width / 2;
    const hh = height / 2;
    return {
        ver: "1.0.27",
...
function buildTtfMeta3x(uuid, imported) {
    return {
        ver: "1.0.1",
        importer: "ttf-font",
        imported: imported !== false,
        uuid,
        files: imported === false ? [] : [".json"],
        subMetas: {},
        userData: {}
    };
}
...
function ensureImageMeta(pngAbsPath, ccVersion) {
    ...
    return writeMetaIfMissing(abs, () => {
        ...
        return ccMajor(ccVersion) === 2
            ...
            : buildImageMeta3x(uuid, baseName, width, height, false);
    });
}
...
function ensureTtfMeta(ttfAbsPath, ccVersion) {
    ...
    return writeMetaIfMissing(abs, () => {
        const uuid = crypto.randomUUID();
        return ccMajor(ccVersion) === 2 ? buildTtfMeta2x(uuid) : buildTtfMeta3x(uuid, false);
    });
}

function ensureMappedFontMetas(fontMapPath, assetsRoot, ccVersion) {
    const empty = { createdMetas: [], copiedFromSystem: [], unavailable: [] };
    if (!fontMapPath || !fs.existsSync(fontMapPath)) return empty;

    const map = fs.readJsonSync(fontMapPath);
    const createdMetas = [];
    const copiedFromSystem = [];
    const unavailable = [];
    ...
    for (const rel of Object.values(map)) {
        ...
        const abs = path.join(assetsRoot, normalized);
        if (!fs.existsSync(abs)) {
            const copyResult = copySystemFontTo(abs);
            if (copyResult.copied) {
                copiedFromSystem.push({
                    rel: normalized,
                    dest: abs,
        ...
        if (fs.existsSync(abs)) {
            ...
            if (result.created) createdMetas.push(abs);
        }
    }

    return { createdMetas, copiedFromSystem, unavailable };
}
