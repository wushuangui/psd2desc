...
const DEFAULT_UI_MATERIAL_2X = "eca5d2f2-8ef6-41c2-bbe6-f9c79d09c432";
...
function emitNode2x(b, ir, parentId, meta, prefabId, pathKey, prefabRootId) {
    const nodeId = b.add({
        __type__: "cc.Node",
        _name: ir.name,
        _objFlags: 0,
    ...
    const prefabRootRef = b.ref(isRoot ? nodeId : prefabRootId);
    const prefabInfoId = b.add({
        __type__: "cc.PrefabInfo",
        root: prefabRootRef,
        asset: isRoot ? b.ref(prefabId) : null,
        fileId: isRoot ? "" : b.fileId(pathKey),
        sync: false
    });
    ...
    for (const child of ir.children || []) {
        ...
        childIds.push(emitNode2x(b, child, nodeId, meta, prefabId, childPath, isRoot ? nodeId : prefabRootId));
    }
